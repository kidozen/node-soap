...
function findInSchema (schema, objectParentName, objectName) {
  if (!schema || !objectParentName || !objectName) {
    return null;
  }

  if (schema[objectParentName] && schema[objectParentName][objectName]) {
    return schema[objectParentName][objectName];
  }
  ...
  var innerSchemasNames = Object.keys(schema.schemas);
  var result;

  innerSchemasNames.forEach(function (innerSchemaName) {
    var innerSchema = schema.schemas[innerSchemaName];
    if (innerSchema[objectParentName] && innerSchema[objectParentName][objectName]) {
      result = innerSchema[objectParentName][objectName];
    }
  });
...
MessageElement.prototype.postProcess = function(definitions) {
  ...
  for (i in children) {
    if ((child = children[i]).name === 'part'
      && child.$name.toLowerCase().indexOf("header") === -1) {
      part = child;
      break;
    }
  }
...
OperationElement.prototype.postProcess = function(definitions, tag) {
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (message.postProcess) message.postProcess(definitions);
    if (message.element) {
...
BindingElement.prototype.postProcess = function(definitions) {
  ...
  if (!portType && type) {
    var schemasNames = Object.keys(definitions.schemas),
      i;

    for (i = 0; i < schemasNames.length; i = i + 1) {
      var schema = definitions.schemas[schemasNames[i]],
        schemaPortType = schema ? schema.portTypes[type] : null;

      if (schemaPortType) {
        portType = schemaPortType;
        definitions = definitions.schemas[schemasNames[i]];
        break;
      }
    }
  }
  ...
  if (portType){
    ...
    for (var index = 0, child; child = children[index]; index++) {
      if (child.name !== 'operation')
        continue;
...
ServiceElement.prototype.postProcess = function(definitions) {
  var children = this.children,
    ...
    schemas = Object.keys(definitions.schemas);
  schemas.forEach(function (schema) {
    var schemaBindings = definitions.schemas[schema].bindings,
      schemaBindingsNames = schemaBindings ? Object.keys(schemaBindings) : [];

    schemaBindingsNames.forEach(function (binding) {
      bindings[binding] = schemaBindings[binding];
    });
  });

    //add definitions.schemas.bindings
  for (var i = 0, child; child = children[i]; i++) {
    if (child.name !== 'port')
      continue;
...
ElementElement.prototype.description = function(definitions, xmlns) {
  ...
  if (type) {
    ...
    var typeName = type.name,
      ...
      schema = definitions.schemas[ns],
      typeElement = schema &&
        ( findInSchema(schema, "complexTypes", typeName)
          || findInSchema(schema, "types", typeName)
          || findInSchema(schema, "elements", typeName) );
  ...
  p.onopentag = function(node) {
    ...
    if (!objectName && top.name === 'Body' && name !== 'Fault') {
      ...
      if (!message) {
        ...
        message = findInSchema(self.definitions, "messages", name);
        ...
        self.definitions.messages[originalName] = message;
      }
...
WSDL.prototype.findParameterObject = function(xmlns, parameterType) {
  ...
  if (this.definitions.schemas) {
    ...
    if (schema) {
      ...
      parameterTypeObj = (schema.complexTypes && schema.complexTypes[parameterType])
        || (schema.elements && schema.elements[parameterType]);
    }
  }
...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
  ...
  parts.push(this.objectToXML(params, null, namespace, xmlns));
  parts.push(['</', namespace, name, '>'].join(''));
